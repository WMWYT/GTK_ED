...
void string_to_hex(unsigned char * des_key, char * key){
	int len = strlen(key);
	unsigned char * strhex = (unsigned char *) malloc(8 * sizeof(char));
	int i, cnt = 0;
	...
	memcpy(des_key, strhex, 8);

	free(strhex);
}

void hex_to_string(char * targect_processed_block, unsigned char * processed_block){
	int tmp = 0;
	int i,j;

	for(i = 0; i < 8; i++){
		
		*targect_processed_block = processed_block[i] / 0x10;
		if(*targect_processed_block >= 0x0A ){
			*targect_processed_block = *targect_processed_block - 10 + 'A';
		}else{
			*targect_processed_block = *targect_processed_block + '0';
		}
		*(targect_processed_block + 1) = processed_block[i] % 0x10;
		if(*(targect_processed_block + 1) >= 0x0A ){
			*(targect_processed_block + 1) = *(targect_processed_block + 1) - 10 + 'A';
		}else{
			*(targect_processed_block + 1) = *(targect_processed_block + 1) + '0';
		}
		targect_processed_block += 2;
	}
}

char * des_encryption_and_deciphering(char * massage, char * key, int process_mode){
	key_set* key_sets = (key_set*)malloc(17*sizeof(key_set));
	int  number_of_blocks, block_count;
	int massage_len = 0;
	unsigned char* data_block = (unsigned char*) malloc(8*sizeof(char));
	unsigned char* data_block_temp = (unsigned char*) malloc(16*sizeof(char));
	unsigned char* processed_block_tmp = (unsigned char*) malloc(8*sizeof(char));
	char * processed_block = (unsigned char *) malloc(16 * sizeof(char));
	unsigned char * des_key = (unsigned char*) malloc(8*sizeof(char));
	
	
	memset(data_block, 0, 8);
	memset(data_block_temp, 0, 16);
	memset(processed_block_tmp, 0, 8);
	memset(processed_block, 0, 16);
	memset(des_key, 0, 8);

	if(strlen(massage) != 16){
		return "明文格式不正确,请输入16位明文";
	}

	if(strlen(key) != 16){
		return "密钥格式不正确,长度不为16";
	}

	string_to_hex(des_key, key);

	generate_sub_keys(des_key, key_sets);
	memcpy(data_block_temp, massage, 16);
	string_to_hex(data_block, data_block_temp);

	process_message(data_block, processed_block_tmp, key_sets, process_mode);

	hex_to_string(processed_block, processed_block_tmp);
